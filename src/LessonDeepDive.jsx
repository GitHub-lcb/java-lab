...
export default function LessonDeepDive({ lessonId }) {
  ...
  return <section className="deep-dive" aria-label="原理精讲">
    <div className="section-title"><h2><BookOpen size={17} aria-hidden="true" />原理精讲</h2></div>
    ...
    <div className="deep-references">{content.links.map(([label, url]) => <a key={url} href={url} target="_blank" rel="noreferrer">{label}<ArrowUpRight size={13} aria-hidden="true" /></a>)}</div>
  </section>;
}
