...
function App() {
  ...
  const openLearningCenter = React.useCallback(() => { if (lab.module && learningModules[lab.module]) selectCenterModule(lab.module); setRoute(true); }, [lab.module, selectCenterModule]);
  ...
  return <div className={`app ${focus ? 'focus-mode' : ''}`}>
    ...
    {route && <Modal className="learning-dialog" title="学习中心" onClose={() => setRoute(false)}><div className="center-module-tabs">{Object.values(learningModules).map(module => <button key={module.name} aria-pressed={centerModule.name === module.name} className={centerModule.name === module.name ? 'active' : ''} onClick={() => selectCenterModule(module.name)}><span>{module.eyebrow}</span><strong>{module.name} 学习中心</strong>{centerModule.name === module.name && <Icon name="Check" size={13} />}</button>)}</div><LearningCenter key={centerModule.name} moduleConfig={centerModule} lessons={centerLessons} completed={completed} assessments={assessments} challengeRuns={challengeRuns} learningState={learningState} onStageSubmit={handleStageSubmit} onReviewAnswer={handleReviewAnswer} onOpenLesson={select} /></Modal>}
  </div>;
}
