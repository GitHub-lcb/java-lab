      ...
      <div className="experiment-main">
        <section ref={workbenchRef} className="workbench" aria-label="实验工作区">
          ...
          <div className={`stage ${tab === 'real' ? 'guided-stage' : ''} ${tab === 'jvm-real' ? 'probe-stage' : ''}`} role="tabpanel" id={`view-${tab}`} aria-labelledby={`tab-${tab}`}>
            {hasRealLab && <div className="guided-host" hidden={tab !== 'real'}><RealRedisLab lesson={lab} onComplete={() => notify('本课真实命令实验已通过')} onReset={() => {}} onProgress={setRealProgress} active={tab === 'real'} /></div>}
...
function App() {
  ...
  return <div className={`app ${focus ? 'focus-mode' : ''}`}>
    <header className="topbar"><a className="brand" href="#redis" onClick={() => select('redis')}><span className="brand-mark"><Icon name="Coffee" size={27} /></span><span>Java <strong>技术实验室</strong><small>JAVA INTERACTIVE LAB</small></span></a><nav className="top-nav" aria-label="主导航"><button className={!route ? 'active' : ''} onClick={() => setRoute(false)}><Icon name="FlaskConical" size={16} />实验室</button><button className={route ? 'active' : ''} onClick={() => setRoute(true)}><Icon name="Route" size={16} />学习中心</button></nav><div className="topbar-actions"><span className="local-status"><i />本地实验环境</span><span className="header-divider" /><IconButton icon={theme === 'dark' ? 'Sun' : 'Moon'} label={theme === 'dark' ? '切换浅色主题' : '切换深色主题'} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} /><IconButton icon={focus ? 'PanelLeftOpen' : 'PanelLeftClose'} label={focus ? '退出专注模式' : '专注模式'} onClick={() => setFocus(!focus)} /><IconButton icon={fullscreen ? 'Minimize' : 'Maximize'} label={fullscreen ? '退出全屏' : '全屏'} onClick={toggleFullscreen} /></div></header>
